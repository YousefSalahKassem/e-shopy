...
- `ui` folder which contains `screens` folder for holding the project's screens files and `widgets` folder for holding shared widgets in the project.
...
to use riverpod you make a variable of the class you want read/watch
example:
```dart
final themeRepoProvider = Provider((ref)=>ThemeRepo());
```
we not use this way and consider to not use it in your code,and make static variable inside the class you want to provide
```dart
class ThemeRepo{
...
```
and to use it in riverpod
```dart
Consumer(
      builder: (context, ref, child) {

        final themeRepo = ref.watch(ThemeRepo.provider);
...
- create event bus
> - use provider to create reference of event bus 
> - use custom StreamSubscription to track event bus and don`t forget to close or pause stream depending on app life ciycle  
> - inject event bus reference inside any place you want to fire data 
